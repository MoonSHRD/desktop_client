...
function router(renderer) {
    ...
    dxmpp.on('online', function (data) {
        ...
        const html = pug.renderFile(__dirname + '/components/main/main.pug', vcard);
        renderer.webContents.send('change_app_state', html);
        ...
        renderer.webContents.send('buddy', {address: address, html: html1});
        // console.log(data);
    });
    //
    ...
    ipcMain.on('get_chat_msgs', (event, arg) => {
        ...
        let html = "";
        if (msgs[arg]){
            msgs[arg].forEach(function (element) {
                html += pug.renderFile(__dirname + '/components/main/messagingblock/outMessage.pug', {
                    message: element,
                }, PUG_OPTIONS);
            });
        }
        renderer.webContents.send('get_chat_msgs', html)
    });
    //
