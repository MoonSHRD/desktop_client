...
class WalletController extends Controller_1.Controller {
    change_wallet_menu(menu_type = "wallet_token") {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (menu_type) {
                ...
                case 'wallet_history':
                    this.send_data('change_wallet_menu', this.render(`main/wallet/${menu_type}.pug`));
                    let txs = yield TransactionModel_1.TransactionModel.getTransactions();
                    for (let i in txs) {
                        let tx = txs[i];
                        tx.amount = TransactionModel_1.TransactionModel.NormalizeValue(tx.amount);
                        ...
                        tx.p_time = Helpers_1.Helper.formate_date(new Date(tx.time), { locale: 'ru', for: 'transaction' });
    ...
    transfer_token(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // console.log(data);
            // return;
            let tx = yield this.web3.SendEth(data.address, data.amount);
            console.log(tx);
            this.send_data('user_joined_room', `Successfully transferred. <br/> txHash: ${tx.transactionHash}`);
            let userModel = yield UserModel_1.UserModel.findOne(data.address);
            let chatModel = yield ChatModel_1.ChatModel.findOne(ChatModel_1.ChatModel.get_user_chat_id(self_info.id, data.address));
