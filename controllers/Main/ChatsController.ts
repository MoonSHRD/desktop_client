...
import {BaseEntity, Column, getConnection, PrimaryColumn} from "typeorm";
...
class ChatsController extends Controller {
    ...
    async joined_room(room_data, messages) {
        ...
        if (room_data.role === 'moderator') {
            ...
            await this.controller_register.run_controller("MessagesController", "get_chat_messages", {
                id: room_data.id,
                type: room_data.type
            })
        }
    ...
    async create_group(group_data) {
        ...
        if (group_data.openPrivate=='on') {
            ...
            let tokenAddress = await this.web3.CreateToken(true, group_data);
            console.log('tokenAddress: ', tokenAddress)
            let group = new ChatModel();
